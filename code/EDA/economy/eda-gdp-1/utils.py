...
def plot_traffic_trend(df, area_name, area_column='local_authority_name', date_column='count_date', vehicle_column='all_motor_vehicles'):
    """
    绘制某个地区的车流量随日期变化的散点图。

    参数：
    - df: DataFrame, 交通数据
    - area_name: str, 需要绘制的地区名称
    - area_column: str, 地区列名
    - date_column: str, 日期列名
    - vehicle_column: str, 车辆总数列名
    """
    # 过滤出指定地区的数据
    df_filtered = df[df[area_column] == area_name].copy()

    # 确保日期列是 datetime 类型，并按日期排序
    df_filtered[date_column] = pd.to_datetime(df_filtered[date_column])
    df_filtered = df_filtered.sort_values(by=date_column)

    # 按日期聚合车流量
    df_grouped = df_filtered.groupby(date_column)[vehicle_column].sum().reset_index()

    # **修正 xlabel 和 ylabel**
    plot_scatter(df_grouped[date_column], df_grouped[vehicle_column], xlabel=f"{date_column}", ylabel=f"{vehicle_column}")
...
def plot_gdp_vs_traffic(df_gdp_common, df_traffic_common, local_authority_name, common_years, traffic_column):
    """
    绘制指定 local_authority_name 在指定年份范围内的 GDP 和交通数据（标准化）

    参数：
    - df_gdp_common: DataFrame, 包含 GDP 数据
    ...
    # **第一部分：GDP 数据处理**
    gdp_data = df_gdp_common[df_gdp_common['local_authority_name'] == local_authority_name]

    # 确保年份列是字符串格式
    years = [str(year) for year in common_years]
    gdp_data = gdp_data[years].transpose()
    gdp_data.columns = ['GDP']
    gdp_data.index = gdp_data.index.astype(int)  # 转换索引为整数年份
    ...
    # **确保年份范围一致**
    gdp_data = gdp_data.loc[common_years]
    traffic_yearly = traffic_yearly[traffic_yearly['year'].isin(common_years)]

    # **标准化数据**
    gdp_data['GDP_normalized'] = zscore(gdp_data['GDP'])
    ...
    # GDP 变化曲线
    sns.lineplot(data=gdp_data, x=gdp_data.index, y='GDP_normalized',
                 marker='o', color='b', label=f'{local_authority_name} GDP (Normalized)', linewidth=2)
    ...
    # **设置标题和标签**
    plt.title(f'{local_authority_name} GDP and {traffic_column} Normalized by Year', fontsize=14)
...
def compute_pearson_correlation(df_gdp_common, df_traffic_common, local_authority_name, common_years, traffic_column):
    """
    计算指定地区的 GDP 和交通数据之间的 Pearson 相关系数，并返回合并后的数据。
    ...
    # **第二部分：转换 GDP 数据（宽表转长表）**
    years = [str(year) for year in common_years]
    gdp_long = gdp_data[years].transpose().reset_index()
    gdp_long.columns = ['year', 'GDP']
    gdp_long['year'] = gdp_long['year'].astype(int)  # 转换年份为整数

    # **第三部分：确保数据年份匹配**
    traffic_data['year'] = traffic_data['year'].astype(int)
    gdp_long = gdp_long[gdp_long['year'].isin(common_years)]
    ...
    # **第五部分：合并数据**
    merged_df = pd.merge(traffic_data, gdp_long, on='year')

    # **第六部分：计算相关系数**
    if len(merged_df) == 0:
        raise ValueError("合并后的数据为空，请检查输入的年份范围和地区名称。")

    corr, p_value = pearsonr(merged_df[traffic_column], merged_df['GDP'])

    print(f"{local_authority_name} 地区的 {traffic_column} 和 GDP 之间的 Pearson 相关系数: {corr:.4f}, P 值: {p_value:.4f}")

    return corr, p_value, merged_df



# 任意城市，任意时间范围，GDP 和 所有交通数据列之间的 Pearson 相关性
# 找出最大相关性的一列
def compute_all_pearson_correlation(df_gdp_common, df_traffic_common, local_authority_name, common_years, traffic_column_indices):
    """
    计算指定地区（如 'Kent'）在给定的年份范围内，GDP 和各交通数据列之间的 Pearson 相关性，返回所有列的相关性结果。
    ...
    for index in traffic_column_indices:
        ...
        corr, p_value = pearsonr(merged_df[column_name], merged_df['GDP'])
...
# 定义封装的函数
def calculate_pearson_correlation(df_traffic, df_gdp, years_range=range(1997, 2023), column_name='all_motor_vehicles'):
    """
    计算各地所有道路交通流量与GDP之间的Pearson相关系数。

    参数：
    df_traffic : DataFrame, 包含交通数据
    df_gdp : DataFrame, 包含GDP数据
    ...
    for la_name in local_authorities:
        ...
        current_gdp_wide = df_gdp[df_gdp['local_authority_name'] == la_name]

        if current_traffic.empty or current_gdp_wide.empty:
            continue  # 如果某个地方的数据为空，则跳过该地方

        years = [str(year) for year in years_range]  # 使用用户自定义的年份范围
        current_gdp_long = current_gdp_wide[years].transpose().reset_index()
        current_gdp_long.columns = ['year', 'GDP']

        # 将年份列转换为整数类型以便后续合并
        current_gdp_long['year'] = current_gdp_long['year'].astype(int)
        current_traffic['year'] = current_traffic['year'].astype(int)

        # 合并两个数据集
        merged_df = pd.merge(current_traffic, current_gdp_long, on='year')

        if merged_df.empty or len(merged_df) < 2:  # 需要至少两个点才能计算相关性
            continue

        # 去除包含NaN或Inf的行
        merged_df = merged_df.replace([np.inf, -np.inf], np.nan).dropna(subset=[column_name, 'GDP'])

        if merged_df.empty or len(merged_df) < 2:  # 再次检查是否还有足够的数据点
            continue

        # 计算交通流量列（如all_motor_vehicles）和GDP之间的相关系数
        corr, p_value = pearsonr(merged_df[column_name], merged_df['GDP'])
...
def plot_traffic_gdp_correlation(df_traffic, df_gdp, years_range=range(1997, 2023), column_name='all_motor_vehicles'):
    """
    Calculate Pearson correlation between traffic data and GDP and plot the results.

    Parameters:
    df_traffic : DataFrame, contains traffic data
    df_gdp : DataFrame, contains GDP data
    ...
    correlation_results = calculate_pearson_correlation(df_traffic, df_gdp, years_range, column_name)
    ...
    plt.title(f'Pearson Correlation between {column_name} and GDP by Local Authority', fontsize=14)
